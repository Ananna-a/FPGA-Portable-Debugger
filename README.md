...
└──────┬──────────┬──────────┬────────────────────────────┘
       ...
       ▼ (命令)   ▲ (应答)   ▲ (ADC 波形)
┌─────────────────────────────────────────────────────────┐
│  FPGA GW5AT-138B (debugger_top.v, ~4200 行)              │
│  FX2_CDC_Core → cdc_cmd_parser → 50+ 命令码路由         │
...
├── src/
│   ├── debugger_top.v         # FPGA 唯一顶层 (~4200 行)
...
| 编号 | 文件 | 内容 |
|------|------|------|
| 00 | `00_文档索引.md` | 文档导航与源码内文档索引 |
| 01 | `01_项目总览.md` | 项目概述、构建命令、模块状态 |
| 02 | `02_系统架构.md` | FPGA ↔ 上位机总体架构、命令链、时钟系统 |
| 03 | `03_通信协议.md` | CDC/UDP 帧格式、50+ 命令码完整表格 |
| 04 | `04_示波器_scope.md` | ① ADC → DDR3 → UDP 数据流详解 |
| 05 | `05_函数发生器_dds.md` | ② DDS 相位累加器 + ROM 查找表 |
| 06 | `06_协议转换器_protocol.md` | ③ 序列 + PWM + CAN + I2C/SPI/DS18B20 |
| 07 | `07_逻辑分析仪_logic_analyzer.md` | ④ 采样/触发 + DSA 数字信号分析 |
| 08 | `08_波特图_bode.md` | ⑤ 波特图 (FPGA 待实现) |
